...
int CCIT_OPT(const Mat1b& img, Mat1i& imgOut) {
    ...
    unsigned char byF = 255;
	Mat1b edgeimage(img.size(), 0);
	Canny_yune(img, 180, 70, edgeimage);
    ...
    const uchar* const img_row = edgeimage.ptr<uchar>(y);
    const uchar* const img_row_fol = (uchar *)(((char *)img_row) + edgeimage.step.p[0]);
    ...
    for (int y = 2; y<h; y += 2) {
        const uchar* const img_row = edgeimage.ptr<uchar>(y);
        const uchar* const img_row_prev = (uchar *)(((char *)img_row) - edgeimage.step.p[0]);
        const uchar* const img_row_fol = (uchar *)(((char *)img_row) + edgeimage.step.p[0]);
        uint* const imgOut_row = imgOut.ptr<uint>(y);
